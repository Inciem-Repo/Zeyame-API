...
<body>
    <div class="container mt-5">
        ...
        @endif
        <form id="imageCategoryForm" method="POST" action="{{ isset($category) ? route('image_categories.update', $category->id) : route('image_categories.store') }}">
            @csrf
            @if(isset($category))
                @method('PUT')
            @endif
           ...
           <a href="{{ route('image_categories.index') }}" class="btn btn-secondary mt-3">Go to Home</a>
    </div>
