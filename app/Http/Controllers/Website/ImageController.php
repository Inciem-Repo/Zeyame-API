...
class ImageController extends Controller
{
    public function uploadJson(Request $request)
    {
       ...
       // Use the S3 disk to store the image in Amazon S3
        ...
        $file_id=$jsonFile->id;
            ...
            $jsonFile = ImageFiles::find($file_id);
            if ($jsonFile) {
                // Update the fields
                // $jsonFile->post_name = $request->input('post_name');
                $jsonFile->post_tags = ",".$request->input('post_tags').",";
                $file_name= 'image_images/'.$temporaryFileName;
                $jsonFile->image_path = 'image_images/'.$temporaryFileName;
                // Save the changes
                $jsonFile->save();
            } else {
    ...
    private function getPostersForCategory($category)
    {
        ...
        // Query ImageFiles for posters matching any descendant category name
        $posters = ImageFiles::where(function ($query) use ($descendantNames) {
    ...
    public function getJsonFiles(Request $request)
    {
        ...
        $cat = Image::with(['children' => function ($query) use ($name) {
    ...
    public function getJsonFilesSearch(Request $request)
    {
        ...
        $cat = Image::select('image.name as name','image.image_path as image_path','image.id as id','image.sort as sort',DB::raw('"0" as type'));
        if($name!=""){
             $cat->where('name', 'like', '%' . $name . '%');
        }
        if($cat_id!=""){
        ...
        $cat->whereIn('name', function ($query) {
            $query->selectRaw("image.name AS tag")
                ->from('json_image')
                ->where('post_tags', 'LIKE', DB::raw("CONCAT('%,', image.name, ',%')"));
        });

       $json_files=Image::select('json_image.post_name as name','json_image.image_path as image_path','json_image.id as id','json_image.sort as sort',DB::raw('"1" as type'))
        ->from('image')
        ->join('json_image', 'json_image.post_tags', 'LIKE', DB::raw("CONCAT('%,', image.name, ',%')"));

        if($name!=""){
             $json_files->where('json_image.post_tags', 'like', '%' . $name . '%');
        }
        
         if (!empty($fill_cat)) {
            $json_files->whereIn('image.name',$fill_cat);
        }
        
        $categories=$cat->unionAll($json_files)->OrderBy('type','asc')->OrderBy('sort','desc')->OrderBy('id','desc')->paginate(30);
 ...
 // Retrieve both parent and child categories matching the search criteria
        $categories = Image::with(['children' => function ($query) use ($name) {
     ...
     public function getJsonFilesSearchv2(Request $request)
    {
       ...
       $json_files=Image::select('json_image.post_name as name','json_image.image_path as image_path','json_image.id as id','json_image.sort as sort',DB::raw('"1" as type'))
        ...
        // return response($categories,200);
         ...
         $data=array();
        $data['categories']=$cat->OrderBy('type','asc')->OrderBy('sort','desc')->OrderBy('id','desc')->get();
         $data['files']=$json_files->distinct()->OrderBy('type','asc')->OrderBy('sort','desc')->OrderBy('id','desc')->paginate(44);
